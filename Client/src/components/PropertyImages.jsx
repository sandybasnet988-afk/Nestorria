...
const PropertyImages = ({property}) => {
    const [hoveredIndex, setHoveredIndex] = useState(0) // initial first image is expanded

    const imageCaptions = [
        {
            heading:"Front View",
            desc:"Experience a stunning first impresstion with elegant curb appeal.",
        },
        {
            heading:"Living Area",
        ...
        },
          {
            ...
            desc:"Fully eqquiped kitchen with sleek finished and modern appliences.",
        },

    ];
  return (
    <div className='flex max-sm:gap-1 max-md:gap-3 gap-5 h-[400px] w-full'>
        {property.images.map((PropertyImages, index) => {
           ...
           return(
            ...
            onMouseLeave={()=> setHoveredIndex(0)} // revert og first image
            >

                <img src={PropertyImages} alt="property" className='h-full w-full object-cover object-center rounded-2xl'/>
